...
| L51 | Mutation enablement gates | Add a tested helper that blocks approved mutations unless workspace, provider, external account, operation type, dry-run status, approval status, rollback metadata, and emergency-stop state all pass. |
...
- approved approval request exists and is not expired;
