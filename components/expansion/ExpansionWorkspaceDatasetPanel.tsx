...
export async function ExpansionWorkspaceDatasetPanel({ compact = false }: ExpansionWorkspaceDatasetPanelProps) {
  try {
    ...
    const [latestSource, snapshots, activeSelection, accounts, offers, policy, runs] = await Promise.all([
    ...
    return (
      <div className={`card lifecycleDatasetPanel ${compact ? "lifecycleDatasetPanel--compact" : ""}`}>
        ...
        <div className="lifecycleDatasetMeta">
          <p><strong>Active source:</strong> {session ? "current expansion app tables. Apply an imported dataset below to replace them, or reset to sample data." : "current expansion sample app tables. Sign in to apply imported workspace data."}</p>
